...
const RoomCard = ({ room }) => {
  ...
  return (
    <div
      ref={cardRef}
      className="relative bg-white shadow hover:shadow-md mx-4 xl:mx-auto max-w-7xl rounded-lg p-4 mt-4 flex flex-col sm:flex-row justify-between sm:items-center"
    >
      ...
      <div className="flex flex-col sm:flex-row w-full sm:w-auto sm:shrink-0 sm:space-x-2 mt-4 sm:mt-0">
        <Link
          href={`/rooms/${room.$id}`}
      ...
      {isBookingOpen && (
        <BookingPopover room={room} onClose={() => setIsBookingOpen(false)} />
      )}
    </div>
  );
};
